...
#   Model ADDONS
    ...
    def getPrioChild(self):
        cdef SCIP_NODE* node = SCIPgetPrioChild(self._scip)
        return Node.create(node)
    
    def getPrioSibling(self):
        cdef SCIP_NODE* node = SCIPgetPrioSibling(self._scip)
        return Node.create(node)
    
    def getBestLeaf(self):
        cdef SCIP_NODE* node = SCIPgetBestLeaf(self._scip)
        return Node.create(node)
    ...
    def executeNodeSel(self, str name): 
        ...
        PY_SCIP_CALL(PyNodeselSelect(self._scip, scip_nodesel, &scip_node))
    ...
    def executeNodeComp(self, str name, Node node1, Node node2):
        ...
        return PyNodeselComp(self._scip, scip_nodesel, node1.scip_node, node2.scip_node)
